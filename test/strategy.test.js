...
describe('Strategy', function() {
  ...
  describe('authorization request with custom nonce and state parameters', function() {
    ...
    it('should be redirected', function() {
      expect(url).to.equal('https://id.smt.docomo.ne.jp/cgi8/oidc/authorize?nonce=the_nonce&authif=1&idauth=1&state=the_state&response_type=code&scope=openid&client_id=ABC123');
    });
  });
  ...
  describe('authorization request with default nonce and state parameters', function() {
    ...
    it('should be redirected', function() {
      expect(url).to.match(/^https:\/\/id.smt.docomo.ne.jp\/cgi8\/oidc\/authorize\?nonce=[a-zA-Z0-9_-]{32}&authif=1&idauth=1&state=[a-zA-Z0-9_-]{32}\&response_type=code\&scope=openid\&client_id=ABC123$/);
    });
  });
});
